...
static struct rte_eth_conf default_port_conf = {
    .rxmode =
        {
            .mq_mode = RTE_ETH_MQ_RX_NONE, .max_lro_pkt_size = RTE_ETHER_MAX_LEN,
        },
...
port_init(lport_t *lport)
{
    ...
    uint32_t num_mbufs  = DEFAULT_MBUF_COUNT;
    uint32_t mbuf_size  = RTE_MBUF_DEFAULT_BUF_SIZE;
    uint16_t cache_size = DEFAULT_CACHE_SIZE;
    int socket_id;
    ...
    if (strcmp(dev_info.driver_name, "net_af_xdp")) {
        lport->rx_mp = lport_pktmbuf_pool("rx", pid, qid, num_mbufs, mbuf_size, cache_size);
        if (lport->rx_mp == NULL)
            rte_exit(EXIT_FAILURE, "Error during allocating mbuf pool for RX\n");

        lport->tx_mp = lport_pktmbuf_pool("tx", pid, qid, num_mbufs, mbuf_size, cache_size);
        if (lport->tx_mp == NULL)
            rte_exit(EXIT_FAILURE, "Error during allocating mbuf pool for TX\n");
    } else {
        lport->rx_mp = lport_pktmbuf_pool("rx", pid, qid, num_mbufs * 2, mbuf_size, cache_size);
        if (lport->rx_mp == NULL)
            rte_exit(EXIT_FAILURE, "Error during allocating mbuf pool for RX\n");

        lport->tx_mp = lport->rx_mp;
	}
