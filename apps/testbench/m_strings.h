 ...
 */
static __inline__ int m_strtok(char *str, const char *delim, char *entries[], int maxtokens)
{
	int i = 0;
	char *saved;
 ...
 *   The number of tokens in the \p entries array.
 */
static __inline__ int m_strqtok(char *str, const char *delim, char *argv[], int maxtokens)
{
 ...
 */
static __inline__ char *m_strtolower(char *str)
{
	if (!str)
		return NULL;
 ...
 */
static __inline__ char *m_strtoupper(char *str)
{
	if (!str)
		return NULL;
 ...
 */
static __inline__ int m_stropt(const char *list, char *str, const char *delim)
{
	...
	if (buf) {
		...
		int nb = m_strtok(buf, delim, argv, STR_MAX_ARGVS);
		if (nb < 0)
			return -1;
		for (int i = 0; i < nb; i++)
 ...
 */
static __inline__ char *m_strdupf(char *str, char *newstr)
{
	if (str)
		free(str);
 ...
 */
static __inline__ int m_strcnt(char *s, char c)
{
	return (s == NULL || *s == '\0') ? 0 : m_strcnt(s + 1, c) + (*s == c);
}
