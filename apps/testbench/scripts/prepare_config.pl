...
sub process_yaml_file
{
    ...
    while ($line = <$fh>) {
	$line =~ s/(TsnHigh|TsnLow|Rtc|Rta|Dcp|Lldp|UdpHigh|UpdLow|L2)Interface: \w+/$1Interface: $inf/g
	    if $inf;

	$line =~ s/(TsnHigh|TsnLow|Rtc|Rta|Dcp|Lldp|L2)Destination: [0-9a-fA-F:]+/$1Destination: $l2_dst/g
	    if $l2_dst;
