...
static void
rtc_initialize_frame(struct thread_context *thread_context __rte_unused, struct rte_mbuf *mbuf)
{
    uint16_t frame_id = app_config.rtc_security_mode == SECURITY_MODE_NONE ? RTC_FRAMEID
                                                                           : RTC_SEC_FRAMEID;
    ...
    initialize_profinet_frame(app_config.rtc_security_mode, rte_pktmbuf_mtod(mbuf, unsigned char *),
                              ...
                              lport_mac_address(app_config.rtc_lport_id),
                              app_config.rtc_destination, app_config.rtc_payload_pattern,
                              app_config.rtc_payload_pattern_length,
                              app_config.rtc_vid | app_config.rtc_pcp << VLAN_PCP_SHIFT, frame_id);
}

static void
rtc_gen_and_send_frames(struct thread_context *thread_context, uint64_t num_frames)
{
    uint64_t sequence_counter = thread_context->tx_sequence_counter;
    lport_id_t id             = app_config.rtc_lport_id;
    ...
    for (int i = 0; i < nb_pkts; i++) {
        ...
        rtc_initialize_frame(thread_context, m);
        ...
        stat_frame_sent(RTC_FRAME_TYPE, sequence_counter + i, tx_time);
...
static void
rtc_tx_thread_routine(void *data, bool signaled)
{
    ...
    if (do_once(&thread_context->tx_do_once))
        ...
        num_frames = app_config.rtc_num_frames_per_cycle;

		/*
         * Send RtcFrames, two possibilities:
         *  a) Generate it, or
         *  b) Use received ones if mirror enabled
         */
        if (!is_mirror_mode())
            rtc_gen_and_send_frames(thread_context, num_frames);
        else
            send_frames_common(RTC_FRAME_TYPE, app_config.rtc_lport_id,
...
static void
rtc_rx_frame(void *data, struct rte_mbuf **mbufs, uint16_t nb_mbufs)
{
    struct thread_context *thread_context = data;
    const unsigned char *expected_pattern = (const unsigned char *)app_config.rtc_payload_pattern;
    ...
    for (uint16_t i = 0; i < nb_mbufs; i++) {
        ...
        if (app_config.rtc_security_mode == SECURITY_MODE_NONE) {
            ...
            prepare_iv((const unsigned char *)app_config.rtc_security_iv_prefix, sequence_counter,
                       &iv);
            ...
            if (ret)
                log_message(LOG_LEVEL_WARNING, "RtcRx: frame[%" PRIu64 "] Not authenticated\n",
                            sequence_counter);
            ...
            if (ret)
                log_message(LOG_LEVEL_WARNING,
                            "RtcRx: frame[%" PRIu64 "] Not authenticated and decrypted\n",
                            sequence_counter);
        ...
        expected_frame_id = app_config.rtc_security_mode == SECURITY_MODE_NONE ? RTC_FRAMEID
                                                                               : RTC_SEC_FRAMEID;
        ...
        if (frame_id_mismatch) {
            log_message(LOG_LEVEL_WARNING, "RtcRx: frame[%" PRIu64 "] FrameId mismatch: 0x%4x!\n",
        ...
        if (payload_mismatch) {
            log_message(LOG_LEVEL_WARNING, "RtcRx: frame[%" PRIu64 "] Payload Pattern mismatch!\n",
                        sequence_counter);
            goto drop;
        }
        ...
        lport_tx_buffer_add(app_config.rtc_lport_id, mbuf);
        continue;
    drop:
...
static void
rtc_rx_thread_routine(void *data, bool signaled __rte_unused)
{
    struct thread_context *thread_context = data;
    uint64_t timo =
    ...
    if (do_once(&thread_context->rx_do_once))
        ...
        int burst_sz = (int)app_config.rtc_num_frames_per_cycle;

        lport_process_pkts(thread_context, burst_sz, app_config.rtc_lport_id, rtc_rx_frame, timo);
    }
}

static int
rtc_threads_routine(void *data)
{
    struct thread_context *thread_context = data;

    rtc_thread_setup(data);

    if (thread_timer_alloc(thread_context, "RTC", MAX_TIMERS) < 0)
        return -1;

    uint64_t curr_ns = clock_gettime_ns();

    if (thread_timer_add(thread_context, RX_TIMER, "RTC-Rx", NULL, rtc_rx_thread_routine, data,
                         curr_ns, app_config.application_rx_base_offset_ns))
        return -1;
    if (thread_timer_add(thread_context, TX_TIMER, "RTC-Tx", &thread_context->data_cond_var,
                         rtc_tx_thread_routine, data, curr_ns,
...
static void
rtc_threads_free(struct thread_context *thread_context)
{
    if (!thread_context)
        return;
    ...
    if (thread_context->payload_pattern) {
        thread_context->payload_pattern -=
            sizeof(struct vlan_ethernet_header) + sizeof(struct profinet_secure_header);
        rte_free(thread_context->payload_pattern);
    }
...
static int
rtc_threads_create(struct thread_context *thread_context)
{
    lport_id_t id = app_config.rtc_lport_id;
    int ret;
    ...
    /* setup DPDK resources */

    thread_context->payload_pattern = rte_calloc_socket(
        "rtcPayload", 1, app_config.rtc_frame_length, RTE_CACHE_LINE_SIZE, rte_socket_id());
    if (!thread_context->payload_pattern) {
        fprintf(stderr, "Failed to allocate RtcPayloadPattern!\n");
        ret = -ENOMEM;
        goto err_exit;
    }
    thread_context->payload_pattern_length = app_config.rtc_frame_length;

    if (app_config.rtc_security_mode != SECURITY_MODE_NONE) {
        thread_context->tx_security_context = security_init(
            app_config.rtc_security_algorithm, (unsigned char *)app_config.rtc_security_key);
    ...
    thread_context->mbufs = rte_calloc_socket("rtcMbufs", MAX_PKT_BURST, sizeof(struct rte_mbuf *),
    ...
    if (tsn_set_vlan_qid(app_config.rtc_vid, lport2qid(id)) < 0) {
...
err_exit:
    rtc_threads_free(thread_context);
    return ret;
}
...
rtc_init(void *arg)
{
    ...
    thread_context->lcore_id = app_config.rtc_thread_cpu;
    return 0;
}
...
rtc_launch(void *arg)
{
    ...
    if (rte_eal_get_lcore_state(thread_context->lcore_id) == RUNNING)
        rte_exit(EXIT_FAILURE, "RTC Lcore %u is already running\n", thread_context->lcore_id);
    return rte_eal_remote_launch(rtc_threads_routine, thread_context, thread_context->lcore_id);
}
...
{
    if (app_config.rtc_enabled)
        rtc_threads_free(arg);
}

FUNCTION_REGISTER(rtc, RTC_IDX);
