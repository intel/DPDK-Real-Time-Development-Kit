...
static int histogram_init(void)
{
	int i;

	if (!app_config.stats_histogram_enabled || is_mirror_mode())
		return 0;
	...
	for (i = 0; i < NUM_FRAME_TYPES; i++) {
		...
		hist->data = rte_calloc_socket("hist", hist->max - hist->min, sizeof(uint64_t), RTE_CACHE_LINE_SIZE, rte_socket_id());
		if (!hist->data)
			return -ENOMEM;
	}
...
static void histogram_free(void)
{
	int i;
...
int histogram_write(void)
{
	...
	fprintf(f, "Testbench RTT Histogram: %s %s %s %s %s %s %s %s %s\n",
		...
		stat_frame_type_to_string(L2_FRAME_TYPE));
...
static int hist_init(void *data __rte_unused)
{
	return histogram_init();
}

static int hist_launch(void *arg __rte_unused)
{
	return 0;
}

static void hist_deinit(void *arg __rte_unused)
{
	histogram_free();
}

FUNCTION_REGISTER(hist, HISTOGRAM_IDX);
