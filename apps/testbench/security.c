 ...
 * equals Output length. Furthermore, PROFINET supports Authentefiaction Only. In that case
...
struct security_context *security_init(enum security_algorithm algorithm, const unsigned char *key)
{
	...
	context = rte_calloc_socket("sec", 1, sizeof(*context), RTE_CACHE_LINE_SIZE, rte_socket_id());
	if (!context)
		return NULL;
