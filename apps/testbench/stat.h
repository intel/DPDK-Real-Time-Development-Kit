...
struct statistics {
	...
	uint64_t rx_sum_ns;
	uint64_t rx_count;
	uint64_t rx_nexttime_sum;
	uint64_t rx_nexttime_cnt;
	double rx_nexttime_avg;
	uint64_t tx_nexttime_sum;
	uint64_t tx_nexttime_cnt;
	double tx_nexttime_avg;
	double rx_timer_avg;
	double tx_timer_avg;
	double txgen_timer_avg;
	bool ready;
};
...
void stat_frame_sent(enum stat_frame_type frame_type, uint64_t cycle_number, uint64_t timestamp);
...
void stat_next_time(enum stat_frame_type frame_type, uint64_t rx_nexttime, uint64_t tx_nexttime);
void stat_timer_add(enum stat_frame_type frame_type, struct stat_avg *avg);

struct statistics *stat_get_global_statistics(enum stat_frame_type frame_type);
void stats_reset_all_stats(void);
