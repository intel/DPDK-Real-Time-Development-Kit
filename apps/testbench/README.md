...
Over the last years the Linux kernel and open source ecosystem introduced TSN functionalities including time synchronization with PTP via 802.1AS, various IEEE traffic shapers, and deterministic frame reception and transmission. The PREEMPT_RT patch turns Linux into a real-time operating system. How well do these mechanisms perform for real world use cases? Is it possible to run PROFINET over TSN on Linux? What jitter, cycle times, throughputs and latencies can be achieved?

To answer these questions, the Linux TSN Testbench was developed by Linutronix GmbH. The purpose is to evaluate manufacturer's hardware as well as underlying drivers and the Linux network stack. Note this is not a TSN conformance testing tool—it is intended for evaluation. The tool is hardware-vendor independent.

This DPDK port (January 2025) replaces the original AF_PACKET/AF_XDP socket implementation with DPDK poll-mode drivers for improved performance and reduced kernel dependencies.

**Original project**: The upstream project has been renamed to Linux RealTime Communication (RTC-Testbench) and is now located at <https://github.com/Linutronix/RTC-Testbench>
...
- Original project (RTC-Testbench): <https://github.com/Linutronix/RTC-Testbench>
