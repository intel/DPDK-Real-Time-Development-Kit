...
print_app_usage(const char *prgname)
{
    ...
    printf("    -c | --cycle-time N     Number of nanoseconds per cycle (e.g., 31250ns = 31.250us)\n");
    ...
    printf("    -s | --link-speed       Desired NIC Link Speed in Mbps (Default Auto-Neg)\n");
    printf("    -R | --run-duration     Amount of time to run 'Hours:Minutes:Seconds' (default forever) (Reference)\n");
    ...
    printf("        --skip-count        Number of packets to skip (Default %d) (Reference)\n", DEFAULT_SKIP_COUNT);
    printf("        --delay-time        Number of seconds to delay (Default: %'d) (Reference)\n", DEFAULT_DELAY_SEC);
    printf("\n");
    // clang-format on
}
...
parse_args(int argc, char **argv)
{
    ...
    // clang-format on
    const char *short_options = "c:b:d:l:Ms:R:PLHh";
    argvopt                   = argv;
    ...
    while ((opt = getopt_long(argc, argvopt, short_options, lgopts, &option_index)) != EOF) {
        ...
        case 'c':        // cycle-time
			...
			printf(">> Cycle Time Set To: %" PRIu64 " ns\n", pinfo->cycle_time_ns);
            break;
        case 'b':        // burst-length
